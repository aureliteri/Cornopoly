...
let rec print_players players = 
  ...
  | h :: t -> let p_id = string_of_int (id h) in 
    ...
    print_endline ("\n" ^ p_name ^ "'s ID is " ^ p_id ^ 
                   " and their current location is " ^ p_cur_loc ^ 
    ...
    begin
      ...
      if (String.equal (property_owner property) "") then 
        begin
          ...
          print_string "> ";
          try_command (read_line()) player playerList
        end 
...
(* LEAVE JAIL SCENARIOS!! 
   like pay the fine, roll a double, or if you have a get out of jail free card
*)


and player_balance_less_0 current_player sp new_acc  =
  let () = print_endline ("Your balance is now " ^ 
                          (string_of_int (balance (current_player)))) in
  let () = print_endline "You have gone bankrupt. You're out of the game!" in 
  let new_board = List.rev(delete_player_record sp (current_player) []) in
  iterate (remove_player current_player playerlist) new_board 
    (fst new_acc, new_board)

and player_balance_greater playerlist sp acc updated_tuple roll new_player 
    current_player new_acc new_t = 
  let updated_sp = snd updated_tuple in 
  if (snd roll) then (** if a double is rolled *)
    begin
      print_endline ("You rolled a double!");
      ...
      if !counter = 3 then (**if the # of doubles is 3, then send the player to jail. *)
        ...
        let jail_player = (change_jail (set_location new_player 10) true )in (**set current location of player to jail. Then change the player's jail property to true  *)
        iterate new_t (updated_sp) (jail_player :: (fst new_acc), updated_sp) (**iterate to next player in line. Add jailed player to accumulator *)
      else iterate (current_player :: new_t) (updated_sp) 
          (fst new_acc, updated_sp) 
          (**if # of doubles rolled per person has not reached three, replace head player with jailed player make  *)
    end 
  else begin
    counter := 0;
...
and not_in_jail_iterate playerlist sp acc player = 
  let roll = roll_dice 6 in 
  let new_player = move player (fst roll) in 
  print_endline (name player ^ " has rolled a " ^ string_of_int 
                   (fst roll) ^ "!");
  let new_space_id = current_location_id new_player in 
  let new_space = get_space new_space_id sp in 
  let updated_tuple = check_space new_space new_player 
      ((fst acc) @ playerlist) sp in
  let current_player = find_player (name player) (fst updated_tuple) in 
  let old_players_list = List.filter (fun x -> id x < id player)
      (fst updated_tuple) in 
  let new_acc = (old_players_list, sp) in 
  let new_t = List.filter (fun x -> id x > id player) (fst updated_tuple) in 
  if (balance (current_player) <= 0) then player_balance_less_0 current_player 
      sp new_acc 
  else 
    player_balance_greater playerlist sp acc updated_tuple roll 
      new_player current_player new_acc new_t

and player_in_jail playerlist sp acc player = (** this is the else helpers *)
  begin (** THE PLAYER IS IN JAIL RIGHT NOW *)
