...
(** [print_locations playerlist acc] iterates through [playerlist] and 
    accumulates the location of each player in the game as a string. *)
...
(** [print_balances playerlist acc] iterates through [playerlist] and 
    accumulates the balance of each player in the game as a string. *)
...
let rec print_properties properties : unit = 
  ...
  | (k,v) :: t -> 
    ...
    print_endline 
      ("Rent Price: $" ^ string_of_int (rent_price k).(v) ^ "\n");
    print_properties t 
...
let rec print_players players board = 
  ...
  | h :: t -> let p_id = string_of_int (id h) in 
    ...
    if List.length p_properties = 0 then print_endline "None"
    else print_properties p_properties; 
...
(** [print_initial_board spaces player] prints all of the information about
    ...
    balance, their current location, and the current list of properties 
    they own. It also includes all of the space names and ids. *)
...
(**[delete_player_record board player acc] deletes all properties of [player] 
   from [board], re-initalizing the owned properties of [player]. 
   This method is called when [player] is eliminated. *)
...
(**[update_board playerlist board] prints all the locations and 
   balances of all players. *)
...
(** [check_space_jail jail player playerList board] updates [player] so that 
    [player] goes to jail and retuns (Player.player list * Space.space list)
     that contains all updated players and spaces. *)
...
(** [check_space_penalty penalty player playerList board] updates 
    [player] according to what [penalty] player landed on and retuns 
    (Player.player list * Space.space list) that 
...
(**[check_space_go chance player playerList board] is the tuple containing
   the updated [playerlist] and [board]. [check_space_go] passes the player
    ...
    that has passed go. *)
let check_space_go (playerList : Player.player list)
    (board : Space.space list) =
...
(**[land_someone_else_property player playerList baord property] updates 
   [player] [board] and [property] according to interaction between [player] 
   and [property]. [player] must first pay rent, then decide if [player] 
   wants to purchase the property from the previous owner. 
   The updated (Player.player list * Space.space list) is returned. *)
...
(**[try_level_property] parses user input [s] and returns
   (Player.player list * Space.space list) according to user's input
...
(**[print_prop_prices player pList board property] displays the current 
   property landed on and the prices and prompts the player to input 
   a command. *)
...
(**[check_space_property space player playerList board property] 
   updates checks if [space] is owned by [player] and returns an updated 
...
let check_space_property space player playerList board property =  
  if String.equal (property_owner property) "" then  
    ...
    else land_someone_else_property player playerList board property 
  end  
    ...
    if in_jail h = false then begin let roll = roll_dice 6 in 
      ...
      if balance current_player <= 0 then 
        ...
        if snd roll then 
          ...
          iterate new_t upd_sp (current_player :: fst new_acc, upd_sp) end 
...
(** [player_in_jail h acc sp t] is the function called when the player [h]
    ...
    try to get out of jail early by satisfying [jail_rules] by 
...
and jail_roll_command player acc sp playerlist = 
  if !(jail_count player) = 3 then 
    ((jail_count player) := 0;
     let not_in_jail = change_jail player false  in
     let () = print_endline "You have stayed in jail for three turns. You are now out of jail."
    ...
    then ((jail_count player) := 0;
      ...
      let () =  print_endline "You didn't roll a double. You are still in jail."
      in iterate playerlist sp (player :: fst acc , snd acc)
  end
...
let end_game lst : unit = 
  ...
  | h :: t -> if List.length t = 0 then 
      (print_endline (name h ^  " is the winner! Everyone else has gone bankrupt. \nThank you for playing Cornopoly! Created by Amy Ouyang, Aaron Kang, Michelle Keoy, Meghana Avvaru."); 
       exit 0; ) 
    else ()
...
(**[print_board_types] prints the Classic vs Dark game board descriptions, 
   to allow the players to decide which board they would like to play. *)
let print_board_type_description () = 
  print_endline "\nYou have a choice of which game board you would like to play.";
  print_endline "Would you like to play the Classic Cornopoly game board? (Easy)";
  print_endline "Classic features: A typical game of Cornolopoly with our favorite \nCornell campus locations as main properties." ;
  print_endline "The properties include CKB, Duffield Hall, Mann Library, and more!";
  print_endline "Or would you rather play the Dark Cornopoly gameboard? (Hard)";
  print_endline "Dark features: A darker game of Cornopoly with the nightmarish version \nof Cornell campus locations as the main properties.";
  print_endline "The properties include Klarman Ghouls, The Deserted Goldwin Smith Hall, \nBecker Remains, and more! The rent prices and buy prices are higher in Dark mode."
...
(**[choose_board] allows the user to select which board type they would like
   ...
   Classic board. It then calls [parse_board_choice] to parse their
   command and will return either the spacelist or spacelist_dark. *)
let choose_board () = 
